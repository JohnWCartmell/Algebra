			...
			<xsl:for-each select="rewriteRule/tt-rule"> 
				...
				<xsl:for-each select="//rewriteRule/tt-rule/tt-conclusion/lhs/*">  <!-- consider each possible outer rewrite rule -->
					...
					<xsl:for-each select="$results">
						...
						<xsl:variable name="topOfDiamondRuleTypeEnriched" as="element(tT-rule)">
						      <xsl:apply-templates select="$topOfDiamondRule" mode="type_enrich"/>
						</xsl:variable>
						<gat:diamond xmlns="http://www.entitymodelling.org/theory/contextualcategory/sequence">
							<gat:from>
								<gat:outer>
									...
									<gat:term><xsl:apply-templates select="$outerTerm" mode="text"/></gat:term>
									<gat:context>
							...
							<gat:top_of_diamond_typing>
							     
							      <!--<xsl:copy-of select="$topOfDiamondRuleTypeEnriched/tT-conclusion/term"/>-->
								  <xsl:apply-templates select="$topOfDiamondRuleTypeEnriched/tT-conclusion/term" mode="text_report_errors"/>
							</gat:top_of_diamond_typing>
							...
							<xsl:if test="true()" >
								...
								<xsl:variable name="left_reduction" as="element(term)">
									<gat:term>
										<xsl:for-each select="$outerTermSpecialised">
								...
								<xsl:variable name="right_reduction" as="element(term)">
									<gat:term>
										<xsl:for-each select="$pointed_outerTerm_specialised">
