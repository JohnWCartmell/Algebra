...
-->
	...
	<xsl:template match="*:seq" mode="lhs">
		<xsl:if test="not(@id)">
			<xsl:message terminate="yes">***************************Something wrong no id atribute in element name(): <xsl:value-of select="name()"/></xsl:message>
		</xsl:if>
		<xsl:if test="@startpos">
			<xsl:text>$</xsl:text>
			<xsl:value-of select="@id"/>
			<xsl:text>_startpos in </xsl:text>
		...
		<xsl:if test="@cardinality">
			<xsl:text>$</xsl:text>
			<xsl:value-of select="@id"/>
			<xsl:text>_cardinality in </xsl:text>
			...
			<xsl:attribute name="select">
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:call-template name="remainder"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
		</xsl:element>
	...
	<xsl:template match="*:seq" mode="lhs_generate_variables">
		<xsl:if test="@startpos">
			<xsl:element name="xsl:variable">
				<xsl:attribute name="name" select="concat(@id,'_startpos')"/>
				<xsl:attribute name="select" select="@startpos"/>
			</xsl:element>
		</xsl:if>
		<xsl:if test="@cardinality">
			<xsl:element name="xsl:variable">
				<xsl:attribute name="name" select="concat(@id,'_cardinality')"/>
				<xsl:attribute name="select">
					<xsl:value-of select="@cardinality"/>
					<xsl:text>[ some $</xsl:text>
					<xsl:value-of select="@id"/>
