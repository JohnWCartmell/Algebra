<xsl:transform version="2.0" 
  ...
  -->
	<xsl:template name="unifySubtermSequences">
		...
		<!--<xsl:variable name="quiet" select="false()"/>-->
		<xsl:if test="not($quiet)">
...
-->
	...
	<xsl:template name="headUnification">
		...
		<xsl:if test="not($quiet)">
			<xsl:message>      headUnification  entry head_substitution entry number: <xsl:value-of select="$head_substitution_entry_number"/>
			</xsl:message>
			<xsl:message>      headUnification  entry subjectTerm   <xsl:apply-templates select="$subjectTerm" mode="text"/>
			</xsl:message>
			<xsl:message>      headUnification  entry targetTerm  <xsl:apply-templates select="$targetTerm" mode="text"/>
			</xsl:message>
		</xsl:if>
		...
		<xsl:variable name="headUnification" as="element(head_substitution)*">
			<xsl:choose>
				<xsl:when test="$subjectTerm/* and not($subjectTerm/*[1][self::*:seq]) and not($targetTerm/*[1][self::*:seq])">
					<!-- source is not seq target is not seq -->
					...
					<xsl:if test="not($quiet)">
						<xsl:message>      headUnification branch 1 subjectTailTerms are <xsl:apply-templates select="$subjectTailTerms" mode="text"/> 
						</xsl:message>
					</xsl:if>
					...
					<xsl:if test="$subjectTerm/*[1][self::*:seq]">
						...
						<xsl:if test="$targetTerm/*[1]">
							...
							<xsl:if test="not(some $targetsubterm 
								...
								satisfies $targetsubterm/name=$subjectseq/name)
								">
								<!--<xsl:message>headUnification case (5a) </xsl:message>-->
								<head_substitution>							
									<gat:substitution>
										<case>5</case>
										<gat:subject>
							...
							<xsl:if test="
								$targetTerm/*[1][self::*:seq]
								and $targetTerm/*[1]/name=$subjectseq/name
								">
								<!--<xsl:message>headUnification case (5b) </xsl:message>-->
								<head_substitution>							
									<gat:substitution>
										<case>5</case>
										<!-- empty substitution -->
		...
		<xsl:if test="not($quiet)">
			<xsl:message>      headUnification exit head substitution entry number: <xsl:value-of select="$head_substitution_entry_number"/>
			</xsl:message>
			<xsl:message>      headUnification exit head substitution subject term: <xsl:apply-templates select="$subjectTerm" mode="text"/>
			</xsl:message>
			<xsl:message>      headUnification exit head substitution  targetTerm:  <xsl:apply-templates select="$targetTerm" mode="text"/>
			</xsl:message>
		</xsl:if>
		<xsl:for-each select="$headUnification"> 
			...
			<xsl:if test="not($quiet)">
				<xsl:message>     headUnification exit detail head substitution result number:<xsl:value-of select="$result_no"/>
				</xsl:message>
				<xsl:message>     headUnification exit detail countOfVariablesSubstituted:<xsl:value-of select="count(substitution/*/substitute)"/>
				</xsl:message>
				<xsl:message>     headUnification exit detail countOfTargetTermsSubstitutedIntoOuter:<xsl:value-of select="count(substitution/*/substitute/term)"/>
				</xsl:message>  
				<xsl:message>     headUnification exit detail subjectTail:<xsl:value-of select="$subjectTail_text"/>
				</xsl:message>
			</xsl:if>
			...
			<head_substitution>
				...
				<xsl:for-each select="substitution"> 
					<gat:substitution>
						...
						<xsl:if test="not($quiet)">
							<xsl:message>   headUnification exit subject substitution <xsl:apply-templates select="subject/substitute" mode="text"/>
							</xsl:message>
							<xsl:message>   headUnification exit target substitution <xsl:apply-templates select="target/substitute" mode="text"/>
							</xsl:message>				
						</xsl:if>
